...
- **当用户在 TTS Preview 完成后说“发来听一下”时，必须在把 Preview MP3 发到对话里的同时，发布该期文字版。**
- **文字版必须嵌入当前这次 TTS Preview 的音频播放器，并提供可直接访问的备用音频链接。**
- Preview 音频必须使用独立 preview R2 路径；不得写入正式 Podcast RSS，不得作为正式 enclosure。
- 文字版使用当前 Preview 对应的 canonical 原文，正文必须逐字一致；禁止摘要、改写、二次生成或删节。
- 此阶段只发布文字站与 preview 音频；**不得发布正式 Podcast，不得写正式 Podcast RSS。**
- 文字站发布后要完成闭环 VERIFY：source entry → preview audio URL reachable → generated page has audio player → Pages deployment success → article URL → 首页/列表 entry。
...
- 正式发布路径：approved artifact → merge/approval state → final Podcast R2 → RSS → metadata VERIFY。
...
- metadata validation 全通过后才允许 final R2/RSS mutation；失败 fail closed。
- 禁用 `SECONDS` 特殊变量；使用 `AUDIO_SECONDS`。复杂逻辑放 Python，不在 YAML command substitution 中嵌 heredoc。

## 7. 新一期与重制是两条流程
- 已存在 GUID 禁止走 normal publish。
- 重制走 `replace-*.json` + `Replace Approved Episode Audio`；GUID/title/article/final R2 URL 不变，只替换 approved MP3，并更新 length/duration。
- 覆盖 R2 前备份旧对象；RSS push 失败必须恢复旧 R2。

## 8. 文字版发布与 VERIFY
- 文字 repo source commit 不等于线上已发布；Pages 是发布链路的一部分，不只是可选验证。
- **文字版正文必须逐字使用当前 Preview 对应的 canonical 口播稿原文，禁止摘要、改写、二次生成或删节。** 网页只允许额外增加 front matter、音频播放器、备用音频链接等展示层内容。
- **Preview 阶段文字版必须带当前 Preview 音频播放器与备用链接。** 缺播放器、URL 不匹配、音频不可访问都视为文字版发布失败。
- Preview 试听阶段文字版不依赖正式 Podcast enclosure；Preview 音频走独立 preview R2 object，不得为了等待正式音频而延迟文字发布。
...
- `draft: false`；Hugo 构建使用 `--buildFuture`，避免 future content 静默过滤。
- VERIFY：source entry → preview audio URL reachable → generated page contains `<audio>` and current preview URL → Pages deployment success → article URL → 首页/列表 entry。
- 文字发布 workflow 必须自身完成 Hugo build + Pages deploy + online verify，不依赖 bot commit 再触发另一个 Pages workflow。

## 9. 最终 VERIFY
Preview/文字交付阶段：
`canonical ✓ → preview artifact ✓ → Preview MP3 发给用户 ✓ → preview R2 ✓ → text source = canonical verbatim ✓ → player uses current preview audio ✓ → Pages ✓ → online article/list ✓`

Podcast 最终发布阶段：
`audio QA/user approval ✓ → approved SHA ✓ → merge/approval ✓ → final R2 ✓ → podcast RSS ✓ → duration/length ✓ → text still matches canonical ✓ → text player switched to final audio ✓ → online article still available ✓`
...
- 临时排障 workflow 用完删除；长期只保留通用 Preview / Preview Text+Audio / Publish Approved Artifact / Replace Approved Episode Audio 入口。
